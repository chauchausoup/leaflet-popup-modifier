...
function roundNumber(number) {
  var tensplace = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 10;
  return Math.round(number * tensplace) / tensplace;
}

var markers = [];
var COORDI = [28.4, 83.99];
...
function addMarker(e) {
  ...
  var newMarker = new L.marker(e.latlng).addTo(leafletMap).bindPopup(popupLayout, markerOptions).openPopup();
  markers.push(newMarker); // i need a sample marker text generator here..
...
var sampleMarkerText = ["<h3>Hospital 1</h3><br>\n     <p>This is hospital 1.</p><br>", "<h1>Hospital 1</h1><br>\n \n     <p>This is hospital 1.</p><br>\n     \n     <button>Hey</button>"];
var popupLayout = sampleMarkerText[0]; //Create a map and assign it to the map div

var leafletMap = L.map('leafletMapid', mapOptions); //on click functionality

newMarkerGroup = new L.LayerGroup();
leafletMap.on('click', addMarker); //  Add a baselayer 
...
var mapBoxOutdoors = L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
  ...
  maxZoom: 18
}).addTo(leafletMap); //here i need to create these 
// Note how popup content can be written as an HTML string, or as JSX within a single node:

L.marker(COORDI).addTo(leafletMap).bindPopup(popupLayout, markerOptions).openPopup(); //lets work with submit button
...
submitMap.addEventListener('click', function () {
  ...
  var feat = getFeaturesInView(); //console.log(feat);
